...
10. **Assess Core Web Vitals.** Measure, report, and optimize the following for every key page/route:
    * Largest Contentful Paint (LCP) — loading performance
    * Interaction to Next Paint (INP) — responsiveness
    * Cumulative Layout Shift (CLS) — visual stability
    * First Contentful Paint (FCP) — perceived load speed
    * Time to First Byte (TTFB) — server/network response time

    For each metric, capture lab data (synthetic testing, e.g. Lighthouse/PageSpeed Insights) and, if real-user data is available (CrUX, RUM/analytics), field data as well. Compare results against Google's published thresholds (Good / Needs Improvement / Poor) and prioritize fixes for any metric in the "Needs Improvement" or "Poor" range.
11. **Run a full Lighthouse/PageSpeed audit for both Mobile and Desktop.** Measure and improve scores across all four categories:
    * Performance
    * Accessibility
    * Best Practices
    * SEO

    For the **Performance** category specifically, capture and optimize these metrics — separately for Mobile and Desktop, since results commonly differ:
    ...
    * Speed Index
    ...
    Record the 0–100 score for each of the four categories on both device types, before and after optimization, and document the specific audits/opportunities flagged by the tool (e.g. render-blocking resources, missing alt text, insecure links, missing meta descriptions) along with the fixes applied.
12. **Validate production behavior.** Verify that:
...
13. **Deliver a comprehensive report**, including:
    ...
    * **Core Web Vitals Report** — before/after values for LCP, INP, CLS, FCP, and TTFB, with Good/Needs Improvement/Poor rating for each.
    * **Lighthouse/PageSpeed Report** — before/after scores for Performance, Accessibility, Best Practices, and SEO, broken out by Mobile and Desktop, plus before/after FCP, LCP, TBT, CLS, and Speed Index for each device type.
